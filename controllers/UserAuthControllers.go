...
func SignupHandler(w http.ResponseWriter, r *http.Request) {
  ...
  var uuid string
  nanoId, nanoIdErr := gonanoid.New(); if nanoIdErr != nil {
	...
	qErr := tx.QueryRowContext(ctx, createUserQuery, data.Username, hashedPassword, data.Email, nanoId).Scan(&uuid); if qErr != nil {
  ...
  ss, tokenErr := token.SignedString(jwtSecret); if tokenErr != nil {
  ...
  body := fmt.Sprintf("<html><body>Verify your account at this link: http://%v/auth/verifyaccount/%v</body></html>", r.Host, ss)
...
func VerifyAccount(w http.ResponseWriter, r *http.Request) {
  ...
  if tokenParseErr != nil {
    ...
    _, qErr := db.DB.Exec(query, claims.Uuid); if qErr != nil {
